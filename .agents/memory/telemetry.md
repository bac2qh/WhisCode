...
## 2026-06-30
- Supersedes the 2026-05-24 default-on decision: app-owned local telemetry is now disabled by default for `whiscode` runtime and `whiscode-enroll`.
- `--telemetry` enables local JSONL telemetry at `~/Library/Logs/WhisCode/events.jsonl`; `--telemetry-path PATH` enables telemetry at a custom path; `--no-telemetry` overrides both opt-in forms.
- Existing telemetry emit sites and CrispASR raw response debug logging remain available only when telemetry is enabled, so default runs do not create or append app-owned telemetry files.
