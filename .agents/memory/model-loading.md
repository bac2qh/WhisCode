...
- CrispASR/VibeVoice is now retired from recommended use and treated as a legacy GGUF compatibility backend. Keep the code path available for existing setups, but prefer `mlx-vibevoice` for local VibeVoice because it avoids the CrispASR server layer, supports VibeVoice context directly, and has been noticeably faster locally.
