...
def test_hotkey_router_ignores_right_option_right_shift_combo():
    router = HotkeyRouter(Key.shift_r)
...
def test_hotkey_router_routes_toggle_key_with_held_key_debounce():
    router = HotkeyRouter(Key.shift_r)
...
def test_manual_hotkey_action_maps_toggle_to_start_or_stop():
    assert manual_hotkey_action(State.IDLE, HOTKEY_TOGGLE_EVENT) == ManualHotkeyAction.START_RECORDING
    assert manual_hotkey_action(State.TRANSCRIBING, HOTKEY_TOGGLE_EVENT) == ManualHotkeyAction.START_RECORDING
    assert manual_hotkey_action(State.RECORDING, HOTKEY_TOGGLE_EVENT) == ManualHotkeyAction.STOP_RECORDING


def test_handsfree_manual_fallback_uses_toggle_for_start_and_finalize():
    assert manual_hotkey_action(State.IDLE, HOTKEY_TOGGLE_EVENT) == ManualHotkeyAction.START_RECORDING
    assert manual_hotkey_action(State.RECORDING, HOTKEY_TOGGLE_EVENT) == ManualHotkeyAction.STOP_RECORDING


def test_final_job_flushes_deferred_send_chunk_batch():
    typed = []
    telemetry = FakeTelemetry()
    deferred_delivery = DeferredTranscriptBuffer()
    first = make_transcription_job(
...
def test_parse_args_rejects_removed_end_hotkey():
    with pytest.raises(SystemExit):
...
def test_help_describes_manual_toggle_hotkey(capsys):
    try:
        parse_args(["--help"])
    except SystemExit as e:
        assert e.code == 0
    ...
    assert "--end-hotkey" not in output
    assert "Send Chunk" not in output
    assert "stop/finalize" in output
