...
When the end phrase stops a recording, WhisCode discards the buffered tail that contains the spoken end phrase before queueing audio for transcription. By default, the tail length is inferred from the enrolled end-phrase WAVs: each readable reference is measured from the first through last sample whose absolute level is at least `--hands-free-active-level`, and the session uses the median valid active span. `--hands-free-tail-seconds FLOAT` remains an explicit override. If no valid active span can be computed, WhisCode falls back to `1.0` second. Right Shift/manual stops and timeout stops still keep the pending tail.
