...
While actively recording, WhisCode reuses the wake phrase as Send Chunk. A confirmed wake phrase during recording trims the spoken wake phrase tail using the inferred active span of the wake references plus `--hands-free-tail-extra-seconds`, queues the chunk to type with a blank line after the transcript, and immediately starts a new recording.
